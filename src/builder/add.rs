...
impl OCIBuilder {
    pub fn add(
        ...
        debug!("add {}  {} to {:?}", cnt_name, src, cnt_rootfs_path);

        self.mount(cnt_name)?;

        let add_id = match utils::copy::add_content(src_path, &cnt_rootfs_path) {
            Ok(dg) => dg,
            Err(err) => {
                self.umount(cnt_name)?;
                return Err(err);
        ...
        let add_id_digest = utils::digest::Digest::new(&add_id)?;

        self.umount(cnt_name)?;

        if src_path.is_dir() {
            history_add_type = "dir";
        }
        ...
        if add_history.to_owned() {
            ...
            let change_history = History {
                ...
                created_by: Some(format!(
                    "/bin/sh -c #(nop) ADD {}:{}",
                    history_add_type, add_id_digest.encoded,
                )),
                comment: None,
